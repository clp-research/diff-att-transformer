...
# TRANSFORMER (SELF-ATTENTION IN IMAGE ENCODER) MODEL
...
class MultiHeadSelfImageAttention(nn.Module):
    ...
    def forward(self, encoded_before_images, encoded_after_images):
      ...
      # compute self-attention on each head
      attention_weighted_stacked_images = [net(image_trajectory[:,:,:,:,index])[0] for index, net in enumerate(self.attention_networks)]
      ...
      # get alphas
      with torch.no_grad():
        alphas = [net(image_trajectory[:,:,:,:,index])[1] for index, net in enumerate(self.attention_networks)]
...
class Encoder(nn.Module):
    ...
    def __init__(self, positional_encoding, nb_heads, pad_length, attention_dim, d_model, n_heads, d_queries,
        ...
        #                                                                 self.nb_heads, self.pad_length, self.d_model)
                                                                              ...
                                                                              # A multi headed image attention layer,
        self.multi_head_image_embedding = MultiHeadSelfImageAttention(self.encoder_dim, self.attention_dim, self.encoded_image_size,
                                                                         self.nb_heads, self.pad_length, self.d_model)
