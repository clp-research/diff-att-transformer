...
# TRANSFORMER (NO ATTENTION IN IMAGE EMBEDDING) MODEL
...
class MultiHeadNoImageAttention(nn.Module):
    ...
    def forward(self, encoded_before_images, encoded_after_images):
      ...
      '''
      # bring encoded images in correct order, then stack input image above for self attention 
      ...
      # blow up trajectory by adding partial changes to the before image until you reach the after image which mimicks a TRANSITION from before to after image
      if self.nb_heads > 1:
      ...
      image_trajectory = torch.stack(image_trajectory, dim=-1) # (batch_size, encoder_dim, h, w, nb_heads)
      # bring into appropriate shape   
      batch_size = image_trajectory.size(0)
      nb_heads = image_trajectory.size(-1)
      image_trajectory_stacked_images = image_trajectory.view(batch_size, -1, nb_heads)  # (batch_size, num_pixels*encoder_dim|flattened_image, nb_heads)
      # swap nb_heads and flattened image in tensor
      image_trajectory_stacked_images = image_trajectory_stacked_images.permute(0, 2, 1)  # (batch_size, nb_heads, flattened_image_size)
      # reduce flattened attention weighted, residualed images to visual words
      output = self.visual_word_net(image_trajectory_stacked_images) # (batch_size, nb_heads, d_model)
      alphas = torch.zeros(batch_size, 1, 14, 14, self.nb_heads)  #(batch_size, 1, h, w, nb_heads) # attention map set to all zeroes due to the fact no attention was applied
...
class Encoder(nn.Module):
    ...
    def __init__(self, positional_encoding, nb_heads, pad_length, attention_dim, d_model, n_heads, d_queries,
        ...
        #                                                                 self.nb_heads, self.pad_length, self.d_model)
                                                                              ...
                                                                              # A multi headed image attention layer,
        self.multi_head_image_embedding = MultiHeadNoImageAttention(self.encoder_dim, self.attention_dim, self.encoded_image_size, 
                                                                         self.nb_heads, self.pad_length, self.d_model)
    ...
    def forward(self, before_images, after_images, encoder_sequence_lengths):
        ...
        number_of_paddings = self.pad_length - self.nb_heads # we take 2*nb_heads because we have the same number attention heads on EACH input image??
