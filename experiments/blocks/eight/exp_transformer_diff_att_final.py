...
config.model_name = "diff_att_8_MODEL_FINAL"
config.transformer_checkpoint = config.save_dir + "/BEST_diff_att_8_MODEL_PRE.pth.tar"  # 'diff_att_8_MODEL_PRE.pth.tar'   # 'transformer_checkpoint.pth.tar'  # path to model checkpoint, None if none
config.start_epoch = 0  # start at this epoch (might be replaced by loaded checkpoint)

# Staged training
config.fine_tune_image_encoder = True  # switch in case of staged training
config.batch_size = 4
